...
class DataDownload:
    ...
    def DownldData_pyJHTDB(self,dataset_name,time,lx,ly,lz,nproc,my_id,auth_token,comm,rank,getFunction='Velocity'):
        ...
        u=np.zeros((lx,ly,lz),dtype='float32')
        v=np.zeros((lx,ly,lz),dtype='float32')
        w=np.zeros((lx,ly,lz),dtype='float32')
        u[:,:,:]=np.transpose(vx)
        v[:,:,:]=np.transpose(vy)
        w[:,:,:]=np.transpose(vz)
        return u,v,w
